...
func Test_processSystemNUMAComputeData(t *testing.T) {
	t.Parallel()
	now := time.Now().Unix()
	store := utilmetric.NewMetricStore()
	...
	testcase := struct {
		...
		getNumaIDFunc     func(l3ID int) (int, error)
		wantNumaBandwidth map[int]float64
	}{
		name:        "single L3MonData, normal arch",
		cpuCodeName: "",
		l3Mon: []malachitetypes.L3Mon{
			{ID: 10, MbmTotalBytes: 1000, MbmVictimBytesPerSec: 500},
		},
		getNumaIDFunc: func(l3ID int) (int, error) {
			if l3ID == 10 {
				return 0, nil
		...
		wantNumaBandwidth: map[int]float64{0: 1000},
	}
	// Patch getNumaIDByL3CacheID for this test case
	mockey.UnPatchAll()
	mockey.Mock(getNumaIDByL3CacheID).To(testcase.getNumaIDFunc).Build()
	systemComputeData := &malachitetypes.SystemComputeData{
		UpdateTime:  now,
		CPUCodeName: testcase.cpuCodeName,
		L3Mon:       malachitetypes.L3Monitor{L3Mon: testcase.l3Mon},
	}
	mmp.processSystemNUMAComputeData(systemComputeData)
	updateTime := time.Unix(now, 0)
	for numaID, want := range testcase.wantNumaBandwidth {
		data, err := store.GetNumaMetric(numaID, consts.MetricTotalMemBandwidthNuma)
		assert.NoError(t, err)
		assert.Equal(t, want, data.Value)
		assert.Equal(t, updateTime, *data.Time)
	}
	// Also check that no unexpected NUMA metrics are set
	for numaID := range testcase.wantNumaBandwidth {
		_, err := store.GetNumaMetric(numaID+100, consts.MetricTotalMemBandwidthNuma)
		assert.Error(t, err)
	}
}
...
// 	numaID, err := getNumaIDByL3CacheID(10)
// 	assert.NoError(t, err)
// 	assert.Equal(t, 0, numaID)
// }
